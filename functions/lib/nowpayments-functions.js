...
exports.nowPaymentsTipWebhook = exports.getNowPaymentsStatus = exports.createNowPaymentsPayment = exports.getNowPaymentsEstimate = exports.getNowPaymentsMinAmount = exports.getNowPaymentsCurrencies = void 0;
...
function getCurrencyDecimals(currency) {
    ...
    if (['btc', 'ltc', 'bch'].includes(lowercaseCurrency))
        return 8;
    if (['eth', 'usdt', 'usdc', 'dai'].includes(lowercaseCurrency))
        return 6;
    if (['usd', 'eur', 'gbp'].includes(lowercaseCurrency))
        return 2;
    return 6;
}
...
exports.getNowPaymentsCurrencies = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        ...
        try {
            ...
            console.log(`✅ Fetched ${((_a = result.currencies) === null || _a === void 0 ? void 0 : _a.length) || 0} supported currencies`);
            response.json({
                success: true,
...
exports.getNowPaymentsMinAmount = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        try {
            const API_KEY = process.env.NOWPAYMENTS_API_KEY;
            if (!API_KEY) {
            ...
            const apiResponse = await fetch(`https://api.nowpayments.io/v1/min-amount?currency_from=${currency_from}&currency_to=${currency_to}`, {
                method: 'GET',
                headers: {
                    'x-api-key': API_KEY,
                    'Content-Type': 'application/json'
...
exports.getNowPaymentsEstimate = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        try {
            const API_KEY = process.env.NOWPAYMENTS_API_KEY;
            if (!API_KEY) {
            ...
            const apiResponse = await fetch(`https://api.nowpayments.io/v1/estimate?amount=${amount}&currency_from=${currency_from}&currency_to=${currency_to}`, {
                method: 'GET',
                headers: {
                    'x-api-key': API_KEY,
                    'Content-Type': 'application/json'
...
// Create payment
exports.createNowPaymentsPayment = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        try {
            const API_KEY = process.env.NOWPAYMENTS_API_KEY;
            if (!API_KEY) {
...
// Get payment status
exports.getNowPaymentsStatus = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        try {
            const API_KEY = process.env.NOWPAYMENTS_API_KEY;
            if (!API_KEY) {
            ...
            const enhancedData = Object.assign(Object.assign({}, payment), { formatted_pay_amount: formatCurrencyAmount(payment.pay_amount || payment.actually_paid, payment.pay_currency), formatted_price_amount: formatCurrencyAmount(payment.price_amount, payment.price_currency), is_completed: ['finished', 'confirmed'].includes(payment.payment_status), is_failed: ['failed', 'expired'].includes(payment.payment_status), is_pending: ['waiting', 'confirming', 'sending'].includes(payment.payment_status), expires_in_minutes: payment.expiration_estimate_date ?
                    Math.max(0, Math.round((new Date(payment.expiration_estimate_date).getTime() - new Date().getTime()) / (1000 * 60))) : null, status_display: getStatusDisplay(payment.payment_status) });
            response.json(Object.assign({ success: true }, enhancedData));
        }
        catch (error) {
...
// IPN Webhook handler
exports.nowPaymentsTipWebhook = firebase_functions_1.https.onRequest(async (request, response) => {
    return corsHandler(request, response, async () => {
        try {
            ...
            const expectedSignature = (0, crypto_1.createHmac)('sha512', IPN_SECRET).update(rawBody).digest('hex');
...
//# sourceMappingURL=nowpayments-functions.js.map