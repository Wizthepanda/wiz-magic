 ...
 */
async function awardXPTransaction(
  transaction: any, 
  userId: string, 
  xpAmount: number, 
  source: string, 
  ...
  if (source === 'watch' && metadata.watchTime) {
    updateData['lifetimeStats.totalWatchTime'] = FieldValue.increment(metadata.watchTime);
    updateData['lifetimeStats.totalVideosWatched'] = FieldValue.increment(1);
  } else if (source === 'completion') {
    updateData['lifetimeStats.totalVideosCompleted'] = FieldValue.increment(1);
  } else if (source === 'share') {
    updateData['lifetimeStats.totalShares'] = FieldValue.increment(1);
  } else if (source === 'referral') {
    updateData['lifetimeStats.totalReferrals'] = FieldValue.increment(1);
  }
 ...
 */
function calculateLevel(currentXP: number): { level: number; progressToNext: number } {
  let level = 1;
 ...
 */
export const awardWizXP = onCall(
  { 
    cors: true,
    region: 'us-central1',
    memory: '512MiB',
  ...
  async (request) => {
    ...
    if (!['watch', 'completion', 'share', 'referral'].includes(source)) {
      throw new HttpsError('invalid-argument', 'Invalid XP source');
    }
    ...
    try {
      return await db.runTransaction(async (transaction) => {
        ...
        if (source === 'watch' && watchTime) {
          updateData['lifetimeStats.totalWatchTime'] = FieldValue.increment(watchTime);
          updateData['lifetimeStats.totalVideosWatched'] = FieldValue.increment(1);
        } else if (source === 'completion') {
          updateData['lifetimeStats.totalVideosCompleted'] = FieldValue.increment(1);
        } else if (source === 'share') {
 ...
 */
export const awardWatchXP = onCall(
  { 
    cors: true,
    region: 'us-central1',
    memory: '512MiB',
  ...
  async (request) => {
    ...
    if (!videoId || typeof watchTime !== 'number' || watchTime < WIZ_XP_CONFIG.MIN_WATCH_TIME) {
      throw new HttpsError('invalid-argument', 'Invalid watch data');
    }
    ...
    try {
      ...
      throw new HttpsError('internal', 'Failed to award watch XP');
    }
  }
);
 ...
 * Award share XP
 */
export const awardWizShareXP = onCall(
 ...
 * Award referral XP
 */
export const awardWizReferralXP = onCall(
  { cors: true, region: 'us-central1' },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
 ...
 * Get user XP data
 */
export const getWizXPData = onCall(
  { cors: true, region: 'us-central1' },
  async (request) => {
    if (!request.auth) {
      throw new HttpsError('unauthenticated', 'User must be authenticated');
    ...
    try {
      ...
      if (!userDoc.exists) {
        ...
        return {
          ...initialData,
          ...levelData,
          dailyXpRemaining: WIZ_XP_CONFIG.DAILY_XP_CAP,
          canEarnXP: true,
        };
      }
      ...
      throw new HttpsError('internal', 'Failed to get XP data');
    }
  }
);
 ...
 */
export const wizDailyReset = onSchedule(
  { 
 ...
 * Get leaderboard
 */
export const getWizLeaderboard = onCall(
  { cors: true, region: 'us-central1' },
  async (request) => {
